...
## Fluxo recomendado de PR (branch `work`)

Para subir novas alterações sem perder contexto:

1. `git fetch origin`
2. `git merge origin/main` (ou `git rebase origin/main`)
3. Resolver conflitos mantendo a versão desejada.
4. `git push -u origin work`
5. Abrir PR `work -> main`.

Em caso de conflito e decisão por manter a versão local:

```bash
git checkout --ours .
git add .
git commit -m "resolve conflitos mantendo versão local"
```

## Ingestão PNCP implementada (v1)

A primeira versão do pipeline já está implementada com:
- janelas de execução (backfill mensal e incremental com reprocessamento),
- extração PNCP com paginação e retry,
- gravação raw em MinIO (`raw/pncp/...`),
- controle operacional no Postgres (`control.etl_runs`, `control.etl_control`) e eventos (`observability.pipeline_events`),
- DAG Airflow integrada ao runner Python.
